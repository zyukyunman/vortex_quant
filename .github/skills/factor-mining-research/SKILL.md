...
默认原则：**除非用户只要求单个已知因子的评测，否则新的量化研究任务都从本 skill 开始。** CogAlpha 是本流程内部的候选生成和进化方法，不是独立策略入口、不是因子本身；`factor-evaluation` 是多周期实证层；`version-review` 是策略 preset 对照层；因子档案是经验沉淀层。
...
读取因子档案和资料线索
  ...
  → 把 qualified/elite 送入 factor-evaluation、策略 overlay 或下一轮 mutation/crossover

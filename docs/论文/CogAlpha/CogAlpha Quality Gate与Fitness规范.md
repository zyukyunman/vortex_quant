...
`vortex/research/cogalpha/` 已实现 Phase 1/2 的最小核心、Phase 2.5 的 Research 集成层、Phase 2.6 的 21-agent baseline proxy 可执行闭环，并在 Phase 2.7 增加语义成熟度治理：

- `schema.py`：`AlphaCandidate`、`LineageRecord`、`QualityIssue`、`QualityCheckResult`、`FitnessStats`、`CogAlphaEvaluationResult`。
- `agent_catalog.py`：七层 21 个 CogAlpha agent 静态 catalog。
- `recipes.py`：21 个 executable baseline proxy recipe；`planned_recipes()` 为空兼容入口；`semantic_status`、`semantic_notes`、`parent_templates` 用于区分代理公式、语义强化代理和变异器代理。
- `adapters.py`：`FormulaSpec` 与 `AlphaCandidate` 的互转/包装。
- `quality.py`：候选元数据检查、字段/算子/危险 token 检查、因子宽表 coverage / inf / distinct / valid dates 检查。
- `fitness.py`：复用 `evaluate_factor`，补 Pearson IC、RankICIR、coverage、distinct ratio、分组单调性、同代相关性过滤、`qualified` / `elite` / `rejected` / `invalid` 决策。
- `workflow.py`：`run_cogalpha_generation()` 串起 recipe、`compute_formula`、quality gate、fitness ranking 和 generation report；`summarize_generation_results()` 输出 agent-level summary。
- `demo.py`：`run_cogalpha_demo()` 用 deterministic synthetic OHLCV panel 跑通 21 个 recipe，并写出 `generation_report.json` / `generation_summary.json`。
- `reports.py`：写出 `vortex.cogalpha_generation_report.v1` JSON。

当前仍未实现：

- LLM provider。
- prompt / guidance 自动生成。
- mutation / crossover 进化 runner。
- 任意公式表达式解析执行。
- CLI、signal snapshot、Strategy / Trade 接入。

### Agent 可执行性的定义

CogAlpha agent 不是只有名字的角色。Vortex 当前规定：

- `catalog` agent：只能用于归因、字段约束和风险提示。
- `planned` recipe：当前为空；若未来出现，表示有研究方向但没有安全 builder，不能参与 `run_cogalpha_generation()`。
- `executable` recipe：同时具备 `AlphaCandidate` 元数据和 `FormulaSpec.builder`，可以计算因子并进入 quality/fitness。
- `semantic_status=proxy`：可执行代理公式，能研究但不能宣称完整复现论文 agent。
- `semantic_status=faithful_proxy`：在当前 OHLCV 字段内对论文 agent 语义做过强化，但仍不是完整 LLM agent。
- `semantic_status=mutation_proxy`：更接近变换/组合/变异器的可执行代理，尤其适用于 `AgentCreative`。

因此，agent 最小实现不是 prompt，而是 recipe。
...
- `nan_or_inf`
- `coverage_too_low`
- `distinct_ratio_too_low`
- `rank_ic_below_rule`
- `rank_icir_below_rule`
  ...
  "results": [
    {
      ...
      "fitness": {
        "primary_horizon": 20,
        "pearson_ic_mean": 0.028,
        "rank_ic_mean": 0.031,
        "rank_icir": 0.42,
        "rank_positive_rate": 0.58,
        "long_short_mean": 0.004,
        "coverage": 0.91,
        "distinct_ratio": 0.32,
        "group_monotonicity": 0.75,
...
`run_cogalpha_demo()` 还会写出 `vortex.cogalpha_generation_summary.v1`：
  ...
  "top_candidates": [
    {
      ...
      "alpha_id": "vtx_cogalpha_market_cycle_relative_trend_60d",
...
Summary 是下一轮 LLM guidance / mutation / archive 的输入层：它不重算指标，只把每个 agent 的 outcome、score、rejection reasons、semantic maturity 和 parent lineage 结构化。
...
2. 补充 Pearson IC、RankICIR、覆盖率、分组单调性和同代相关性过滤。
3. 保持 `FactorEvaluationResult` 可序列化。
4. 不破坏现有测试和报告 schema。
5. MI 暂不进入最小核心，避免过早引入离散化口径争议。

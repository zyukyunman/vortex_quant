...
> 本文档回答一个很实际的问题：Vortex 对外到底让用户配置什么，`profile` 到底是什么，它和 `snapshot`、`strategy`、`workflow` 的关系又是什么。

---

## 一、背景

在前面的设计里，我们已经把使用者视角收敛到了 4 个核心对象：
...
但如果不继续把 `profile` 讲清楚，系统仍然会退化成“看起来有统一命令，实际还是一堆脚本参数”。

真正的问题不是“有没有 profile”，而是：

1. `profile` 到底承载哪些用户可见参数。
2. `profile` 和数据版本、研究流程、策略回测、交易执行是什么关系。
3. `profile` 是不是应该做成一个超级大 YAML，把所有配置都塞进去。

本文的结论很明确：

1. `profile` 必须是用户的第一配置对象。
2. 但 `profile` 不能是一个巨大的全局配置文件。

...
| 2 | 运行 `vortex strategy earnings-forecast prepare-next-session --preset baseline_top110_large` | 写出 `trade_date=today` 的冻结组合与 pending task；信号日必须早于执行日；候选池按信号日 `valuation.total_mv` 只保留 A 股总市值前 50%；`stock_st` 日度名单、财务 ST 风险和名称含 ST 任一命中都剔除 | 任一关键数据缺失则 fail-closed |

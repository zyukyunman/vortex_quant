...
6. `timezone`：时区。
7. `calendar`：交易日历。
8. `quality_pack`：质量门禁策略。
9. `pit_pack`：财务 PIT 策略。
10. `publish_pack`：快照发布时间、命名与保留策略。
...
2. 常用可选字段可能是 `datasets`、`universe`、`publish_pack.publish_time`。
3. 高级可选字段可能是某些质量阈值。
...
2. **平台内部层**：由 preset 展开出 `provider`、`datasets`、`quality_pack`、`pit_pack`、`publish_pack`、`storage_pack` 等技术字段。
...
### 11.4 Preset/Pack 加载与验证

> 产品冻结文档 §八 定义了三层配置模型：用户层（profile YAML）→ 预设层（preset）→ 包层（pack）。`ProfileLoader` 只负责用户层 YAML，preset/pack 的加载与验证由以下组件承接。

文件建议：`vortex/config/preset/loader.py`

1. `PresetLoader`

职责：

1. 根据 profile 中声明的 `preset` 名称，查找并加载对应的预设定义。
2. 展开 preset 中引用的 pack（`quality_pack`、`pit_pack`、`publish_pack`、`storage_pack` 等）。
3. 返回一个"已展开但未做运行时解析"的中间对象。

文件建议：`vortex/config/preset/pack_registry.py`

1. `PackRegistry`

职责：

1. 管理所有已注册的 pack 定义（内置 pack + 用户自定义 pack）。
2. 按名称查找 pack 并返回其配置字典。
3. 校验 pack 内字段合法性（如 `quality_pack` 的规则列表是否存在）。

文件建议：`vortex/config/preset/validator.py`

1. `PresetValidator`

职责：

1. 校验 preset 引用的 pack 是否全部存在。
2. 校验 pack 之间无冲突（如 `storage_pack` 与 `publish_pack` 的路径兼容性）。
3. 校验 preset 展开后的完整字段集是否满足对应 profile 类型的最低要求。
...
class PresetLoader:
  def load(self, preset_name: str) -> "ResolvedPreset": ...
  def expand_packs(self, preset: "ResolvedPreset") -> dict[str, Any]: ...


class PackRegistry:
  def get(self, pack_name: str) -> dict[str, Any]: ...
  def list_packs(self, category: str | None = None) -> list[str]: ...
  def validate(self, pack_name: str) -> None: ...
...
4. `PresetLoader` 根据 profile 中声明的 `preset` 加载预设，`PackRegistry` 展开各 pack。
5. `ProfileMerger` 展开 `extends`，合并默认层、preset 层、实例层、runtime override。
6. `ProfileValidator` 检查字段合法性和命令空间匹配性。
7. `SnapshotSelector` 解析 snapshot 别名。
8. `ProfileResolver` 产出 `ResolvedProfile`。
9. 业务模块只消费 `ResolvedProfile`，不再关心 YAML 合并和 pack 展开细节。
